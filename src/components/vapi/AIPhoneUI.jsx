          ...
          <div className="voice-grid grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            {voices.map((voice) => (
              <button
                ...
                className={`voice-btn p-2 md:p-3 border rounded-md flex flex-col items-center text-gray-700 text-sm md:text-lg transition ${
                  selectedVoice === voice ? "border-[#283CFF] bg-[#283CFF] text-white shadow-md" : "border-gray-300 hover:bg-gray-200"
                }`}
              >
        ...
        <div className="phone relative bg-gray-900 w-full max-w-xs md:w-72 h-[500px] md:h-[545px] rounded-3xl border-4 border-[#283CFF] shadow-lg flex flex-col items-center justify-center p-4">
          {!answered ? (
            <>
              ...
              <div className="slide-btn-container absolute bottom-4 md:bottom-6 w-full flex justify-center">
                <div
                  ref={trackRef}
                  className="slide-btn relative w-full max-w-sm md:w-64 h-12 md:h-14 rounded-full flex items-center justify-center text-white font-bold overflow-hidden bg-gray-700 transition-all duration-200"
                >
                  ...
                  <div
                    ref={sliderRef}
                    className="slide-btn-inner absolute left-0 w-10 h-10 md:w-12 md:h-12 bg-white rounded-full flex items-center justify-center cursor-pointer transition-transform duration-200"
            ...
            <p className="text-white text-center text-lg md:text-xl font-bold">Connecting...</p>
          ) : (
            <div className="text-white text-center">
              <h2 className="text-lg md:text-xl font-bold">Call Connected</h2>
              <p className="text-sm md:text-lg font-semibold mt-2">Time Left: {formatTime(timer)}</p>
              <button className="call-end-btn bg-red-500 text-white px-4 py-2 rounded-md flex items-center justify-center" onClick={() => {
                setAnswered(false);
                endCall();
              }}>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
